...
class PointOfViewController {
  cardsService: any;
  povContainerCardsDistance: number;
  povContainerSide: number;
  controlsContainerScale: number;
  cardsHeight: number;
  spaceBetweenCards: number;
  maxSpaceBetweenCards: number;
  minSpaceBetweenCards: number;
  previousSpaceBetweenCards: number;
  cardsWidthHeightRatio: number;
  povX: number;
  povY: number;
  cardProfileWidth: number;
  cornerX: number;
  cornerY: number;
  controlsVisible: boolean;
  windowHeight: number;
  boundMovePov: any;
  moveCardProfileFunction: any;
  cardProfileClickPoint: any;
  ...
  updateCards() {
    ...
    // cards container has the same dimensions as the first card
  ...
  setBordersMode(isBordersMode: boolean) {
    ...
    if (isBordersMode) {
      cardsElements.forEach((cardElement: HTMLElement) => cardElement.classList.add('hidden-img'));
      imgsElements.forEach((imgElement: HTMLElement) => imgElement.classList.add('hidden-img'));
      ...
      cardsElements.forEach((cardElement: HTMLElement) => cardElement.classList.remove('hidden-img'));
      imgsElements.forEach((imgElement: HTMLElement) => imgElement.classList.remove('hidden-img'));
  ...
  onCardProfileMouseDownFactory(k: number) {
  ...
  moveCardProfileFactory(k: number) {
    const f = (event: MouseEvent) => {
